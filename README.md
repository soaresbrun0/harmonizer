...
The device page also carries a `problem` sensor (`Device Health` — reports off every interval when healthy, expires to unknown after a fixed timeout without updates), one sensor per trendable signal so HA can graph each (`Heap` with heap size, low-water mark, largest allocatable block, and free percent as attributes; `CPU`: loop rate in Hz with core count and clock as attributes; `Flash` with total flash, free sketch space, and free percent as attributes; `Uptime` in seconds; `PSRAM` when the board has PSRAM; `Wi-Fi`: RSSI plus SSID/BSSID/IP, also expiring on the same fixed timeout), an `Other Device Attributes` sensor carrying the remaining static chip facts (model, revision, SDK), a `Pressed Buttons` sensor showing the currently held buttons, `+`-joined and sorted for chords (empty when idle), for live remote verification, and a `Restart` button that reboots the MCU a second after being pressed.
Metric reports pause while the remote is in use and resume after a quiet period (`HA_METRICS_REPORT_RESUME_DELAY_MS`), so publish bursts never stall button handling.
Last known values persist while offline. This template binary sensor turns quiet into a visible problem:
